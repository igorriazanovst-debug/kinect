...
$gesturesPath  = "$projectPath\Assets\Scripts\Gestures"
...
$gm = Get-Content $gmPath -Raw -Encoding UTF8

$gm = $gm -replace '(\[SerializeField\] private float _stepThreshold\s*=\s*)[\d.]+f', '${1}0.30f'
$gm = $gm -replace '(\[SerializeField\] private float _turnThreshold\s*=\s*)[\d.]+f', '${1}0.25f'
$gm = $gm -replace '(\[SerializeField\] private float _holdDuration\s*=\s*)[\d.]+f',  '${1}0.50f'
$gm = $gm -replace '(\[SerializeField\] private float _cooldown\s*=\s*)[\d.]+f',      '${1}1.00f'

Set-Content -Path $gmPath -Value $gm -Encoding UTF8
Write-Host "[OK] GestureManager.cs updated"
...
$debugScript = $lines -join "`r`n"
Set-Content -Path "$editorPath\GestureDebugWindow.cs" -Value $debugScript -Encoding UTF8
Write-Host "[OK] GestureDebugWindow.cs created"
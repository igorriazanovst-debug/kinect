...
namespace EduMotion.Editor
{
    public class GameScenesSetup
    {
        ...
        private static void SetupTraffic()
        {
            ...
            // Score
            var scoreText = CreateText(canvasGO, "ScoreText", "Очки: 0", 28, new Vector2(0f, 0.9f), new Vector2(0.4f, 1f));
            ...
            // Instruction
            var instrText = CreateText(canvasGO, "InstructionText", "Смотри на светофор!", 32, new Vector2(0.1f, 0.3f), new Vector2(0.9f, 0.44f));

            // Feedback
            var feedbackText = CreateText(canvasGO, "FeedbackText", "", 40, new Vector2(0.1f, 0.15f), new Vector2(0.9f, 0.3f));

            // Back button
            var backBtn = CreateButton(canvasGO, "BackButton", "← Меню", new Vector2(0f, 0.9f), new Vector2(0.2f, 1f));
        ...
        private static void SetupCrossing()
        {
            ...
            CreateText(canvasGO, "ScoreText",       "Очки: 0",              28, new Vector2(0f,   0.9f), new Vector2(0.4f, 1f));
            CreateText(canvasGO, "InstructionText", "Шагни вперёд!",        32, new Vector2(0.1f, 0.4f), new Vector2(0.9f, 0.55f));
        ...
        private static void SetupEcology()
        {
            ...
            CreateText(canvasGO, "ScoreText",       "Очки: 0",            28, new Vector2(0f,   0.9f), new Vector2(0.4f, 1f));
            CreateText(canvasGO, "InstructionText", "Сортируй мусор!",    32, new Vector2(0.1f, 0.1f), new Vector2(0.9f, 0.25f));
...
Set-Content -Path "$editorPath\GameScenesSetup.cs" -Value $script -Encoding UTF8
Write-Host "GameScenesSetup.cs created: $editorPath\GameScenesSetup.cs"

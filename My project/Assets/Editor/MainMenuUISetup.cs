...
namespace EduMotion.Editor
{
    public class MainMenuUISetup
    {
        ...
        {
            // РћС‚РєСЂС‹РІР°РµРј СЃС†РµРЅСѓ MainMenu
            ...
            var statusText = CreateText(statusPanel, "KinectStatusText", "Kinect: РЅРµ РЅР°Р№РґРµРЅ", 20, FontStyle.Normal);
            SetRect(statusText, new Vector2(0.18f, 0.05f), new Vector2(0.95f, 0.95f), Vector2.zero, Vector2.zero);

            // Buttons
            var trafficBtn  = CreateButton(canvasGO, "TrafficButton",  "рџљ¦ РЎРІРµС‚РѕС„РѕСЂ",    new Vector2(0.1f, 0.42f), new Vector2(0.9f, 0.58f));
            var crossingBtn = CreateButton(canvasGO, "CrossingButton", "рџљё РџРµСЂРµС…РѕРґ",     new Vector2(0.1f, 0.24f), new Vector2(0.9f, 0.40f));
            var ecologyBtn  = CreateButton(canvasGO, "EcologyButton",  "в™»пёЏ Р­РєРѕР»РѕРіРёСЏ",   new Vector2(0.1f, 0.06f), new Vector2(0.9f, 0.22f));
